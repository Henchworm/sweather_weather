...
RSpec.describe WeatherService do
  it "returns weather data for provided coords", :vcr do
    data = [
...
318A-1&scalebar=true&zoom=12&rand=-964167501"},
  ...
  weather_result = WeatherService.get_forecast(coord_1)
  expect(weather_result).to be_a(Hash)
  expect(weather_result).to have_key(:current)
  expect(weather_result).to have_key(:hourly)
  expect(weather_result).to have_key(:daily)
  end
end